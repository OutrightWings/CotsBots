    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        if (mIsColorSelected) {
            ...
            for (MatOfPoint contour : contours) {
                ...
                double objectArea = rect.width * rect.height;
                double frameArea = mRgba.cols() * mRgba.rows();
                double areaRatio = objectArea / frameArea;


                String ratioText = String.format("Area: %.1f%%", areaRatio * 100);
                Imgproc.putText(mRgba, ratioText, new Point(50, 150),
                        Imgproc.FONT_HERSHEY_SIMPLEX, 1.0, new Scalar(255, 255, 0), 2);

                // circle center point
                Imgproc.circle(mRgba, center, 10, new Scalar(0, 255, 0), -1);
                Log.i(TAG, "Green object center: (" + center.x + ", " + center.y + ")");
                ...
                if (currentTime - lastSendTime >= SEND_INTERVAL) {
                    if (areaRatio >= 0.70) {
                        ...
                        if (connectedThread != null) {
                            connectedThread.write("x");  // 停止
                        }
                    }
                    else {

                        if (center.x < frameWidth / 2.0) {
                            Log.i(TAG, "Ball Position: LEFT");
                            Imgproc.putText(mRgba, "LEFT", new Point(50, 100),
                                    Imgproc.FONT_HERSHEY_SIMPLEX, 1.5, new Scalar(0, 255, 0), 3);
                            if (connectedThread != null) connectedThread.write("a");
                        } else {
                            Log.i(TAG, "Ball Position: RIGHT");
                            Imgproc.putText(mRgba, "RIGHT", new Point(50, 100),
                                    Imgproc.FONT_HERSHEY_SIMPLEX, 1.5, new Scalar(0, 255, 0), 3);
